"""
Generierung für ATOS Klinik Stuttgart GmbH
Standort: Stuttgart
Rolle: OTA (Operationstechnische/r Assistent/in)
...
async def generate_all_creatives():
    """Generiert alle 6 Creatives für ATOS Stuttgart"""
    
    print("=" * 80)
    print("ATOS KLINIK STUTTGART - 6 CREATIVES (OTA)")
    print("=" * 80)
    
    company_name = "ATOS Klinik Stuttgart GmbH"
    location = "Stuttgart"
    job_title = "Operationstechnische/r Assistent/in (m/w/d)"
    primary_color = "#004B87"  # ATOS Blau
    ...
    personas = [
        {
            "name": "Die strukturierte Frühdienst-OTA",
            "hook": "Dein OP-Tag beginnt planbar – und endet pünktlich.",
            "subline": "Frühdienst. Planbare OP-Zeiten. Pünktlicher Feierabend.",
            "cta": "Jetzt bewerben",
        ...
        {
            "name": "Die großhausmüde Spezialistin",
            "hook": "Kleines OP-Team, klare Eingriffe, echte Ruhe.",
            "subline": "Orthopädie-Fokus. Strukturierte Abläufe. Wertschätzung.",
            "cta": "Mehr erfahren",
        ...
        {
            "name": "Die verlässliche Teamstütze",
            "hook": "Ein OP, ein Team – ohne tägliche Neuordnung.",
            "subline": "Stabilität. Feste Kollegen. Keine Springer-Einsätze.",
    ...
    for i, persona in enumerate(personas, 1):
        ...
        pro_desc = "clean modern photography, professional operating room setting, medical precision, natural lighting"
        pro_brief = await brief_service.generate_brief(
            headline=persona['hook'],
            style=f"professional, precise, trustworthy, medical excellence, VISUAL STYLE: {pro_desc}",
        ...
        art_desc = "watercolor painting, soft brush strokes, professional blue tones, calm focused atmosphere"
        art_brief = await brief_service.generate_brief(
            headline=persona['hook'],
            style=f"calm, professional, caring, focused precision, ARTISTIC RENDERING: {art_desc}",
            subline=persona['subline'],
            benefits=[],
            job_title=job_title,
            cta=persona['cta']
    ...
    print(f"{'='*80}")
    print("\n[SUCCESS] ATOS Stuttgart OTA Generierung abgeschlossen!")
