...
async def generate_all_creatives():
    """Generiert alle 6 Creatives für VidaCura Bremen"""
    
    print("=" * 80)
    print("VIDACURA GMBH BREMEN - 6 CREATIVES (Pflegefachkraft)")
    print("=" * 80)
    
    company_name = "VidaCura GmbH"
    location = "Bremen"
    job_title = "Pflegefachkraft (m/w/d)"
    primary_color = "#0066A1"  # Professionelles Pflege-Blau
    ...
    personas = [
        {
            "name": "Die klinikmüde Stadt-PFK",
            "hook": "Raus aus dem Klinikstress – rein in verlässliche Dienste",
            "subline": "Planbarkeit. Nähe. Feste Teams statt Dauerdruck.",
            "cta": "Jetzt bewerben",
        ...
        },
        {
            ...
            "hook": "Wiedereinstieg mit festen Tagen und echtem Rückhalt",
            "subline": "Teilzeit. Sicherheit. Ohne Schuldgefühle oder Chaospläne.",
            "cta": "Mehr erfahren",
        ...
        {
            "name": "Die wechselbereite Struktur-Sucherin",
            "hook": "Ein Standort, ein Team, ein klarer Plan",
            "subline": "Klarheit. Führung. Struktur statt Unordnung.",
    ...
    for i, persona in enumerate(personas, 1):
        ...
        pro_desc = "clean modern photography, professional nursing home setting, organized care environment, natural lighting, calm supportive atmosphere"
        pro_brief = await brief_service.generate_brief(
            headline=persona['hook'],
            style=f"professional, trustworthy, organized, calm, reliable, VISUAL STYLE: {pro_desc}",
        ...
        art_styles = [
            "watercolor painting, soft brush strokes, warm caring colors, peaceful atmosphere, artistic illustration",
            "3D clay render, soft rounded shapes, pastel healthcare colors, supportive and gentle, friendly Pixar style",
            "neon glow aesthetic, dark background, vibrant professional colors, modern, urban Bremen cityscape with Weser river"
        ]
        art_desc = art_styles[i-1]
        art_brief = await brief_service.generate_brief(
            headline=persona['hook'],
            style=f"warm, supportive, caring, reliable professional atmosphere, empowering, ARTISTIC RENDERING: {art_desc}",
            subline=persona['subline'],
            benefits=[],
            job_title=job_title,
            cta=persona['cta']
    ...
    print(f"{'='*80}")
    print("\n[SUCCESS] VidaCura Bremen Generierung abgeschlossen!")
